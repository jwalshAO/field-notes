...
Last updated: 2026-05-25 | Session #17 (extended) (V4.1.6 + V4.1.7 + V4.1.8 + V4.1.9 + Q v19 + Field Guide V4.2.0 schema — Tony Matteo fix → modal copy overhaul → lock/share visibility per note → visibility-at-capture toggle → Field Guide tables rebuilt)
...
## 9. History Log
- **2026-05-25 — Session 17 (continued, end of day): V4.1.8 + V4.1.9 + intro modal copy + Field Guide V4.2.0 schema.**
  - **V4.1.8 lock/share visibility shipped end-to-end.** Backend scaffolding existed (column `field_notes.is_shared` + `reps.default_notes_shared` + SELECT RLS referencing is_shared) but UI never built. Added: `set_field_notes_is_shared_default` BEFORE INSERT trigger that fills value from rep's default when not explicitly passed; new `derived_facts.is_shared` column + sync trigger from source note; rewrote `auth_derived_facts_read` RLS (was `USING (true)` — locked facts now hide from other reps); `update_note_visibility(p_note_id, p_is_shared)` RPC that cascades to derived_facts; PWA: 🌐/🔒 toggle icon per note in Notes Feed with optimistic update + revert-on-fail. **Verified end-to-end** with INSERT trigger tests: Noah row → is_shared=true, John row → is_shared=false. John confirmed icons appear in Notes Feed.
  - **V4.1.9 visibility-at-capture toggle.** Per John's feedback after V4.1.8 launched: "should be on the note creation flow, not just retroactively." Added a tappable visibility row in the confirm modal (between Next Action and Submit buttons): icon + label + sub. Pre-selected from `currentUser.default_notes_shared` (or the existing draft's is_shared on resume). `confirmIsShared` state variable; `renderConfirmVisibility` + `toggleConfirmVisibility` functions; `finalizeNote` passes is_shared explicitly so the trigger doesn't override.
  - **Intro modal copy overhaul.** John eyeballed the V4.4.x intro modal and asked for several rewrites: "Agility brain" → "the team dossier" (Bond-themed, threads the Q-Branch reference); added italic Q-origin sentence ("Why 'Q'? After Bond's Q-Branch — your technical sidekick in the field..."); "dashboard" → "CRM" in the pipeline description; Schedule signals example rewritten to "Boz is at PMUC on Mondays. Builds out the surgeon calendar." (also exercises the new V4.1.7 nickname); Product interest broadened from Agility-only product names to include competitors and clinical categories ("competitors, Geminus, nerve wraps, distal humerus"); Competitor sightings made vivid ("tray on the loaner rack, rep talking to a surgeon, Dr. Strange using Acumed for clavicle"); Tend example changed from "Coffee with Battista" to "Chatting with Battista"; G2G philosophy modal got BD acronym defined on first use + "pushing trays" → "pushing for sales" framing; help "?" button restyled from gray text-only to small solid-blue circle with white "?". SW cache progression v4-27 → v4-28 → v4-29 → v4-30.
  - **Field Guide V4.2.0 schema rebuilt (frontend pending).** Triggered by John's question: "how are we doing hospital notes and surgeon preferences — is that part of Field Notes?" Honest answer: capture path was never built after Session 10's V4.2.0 reversal. Discovered the original Session 10 tables had been DROPPED but a DIFFERENT table named `surgeon_preferences` now exists for the **Geminus Playbook** (structured technique forms with the 2.5/3.5 screw checkbox added Session 16). Decision: don't collide — rename the new Field Guide table to `surgeon_field_guide`. The two systems coexist: `surgeon_preferences` for structured per-procedure playbooks, `surgeon_field_guide` for free-text categorical preferences. Migration `field_guide_v4_2_0_schema_renamed` shipped both `surgeon_field_guide` (surgeon × procedure_code × preference_category {positioning, fluoro, instrumentation, tray_setup, scrub_tech, personality, donts, other} × body) and `location_field_guide` (location × guide_category {parking, access, locker_room, key_people, supplies, or_specific, hours, wifi, other} × body) with UNIQUE constraints for overwrite semantics, RLS (all authenticated reps read+write, admin delete), and `upsert_surgeon_field_guide` + `upsert_location_field_guide` SECURITY DEFINER RPCs that auto-set updated_by_rep_id from auth.uid() and delete the row on empty body. **Frontend (Neo CRM tabs on /surgeon/[id] and /hospital/[id]) deferred to a fresh session tomorrow** — substantial enough build that it warrants a clear head.
  - **Commit + push** to field-notes repo with V4.1.8 + V4.1.9 + all modal copy + SW cache v4-30. Vercel auto-deploy.
